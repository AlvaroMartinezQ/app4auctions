<template>
  <div>
    <p class="text-h5 q-my-sm">Your buy processes</p>
    ...
    <q-card v-for="entry in records" :key="entry.id" class="q-my-md">
      <q-card-section class="bg-secondary text-white">
        Buy process unique number identifier {{ entry.id }}
      </q-card-section>
      <q-card-section class="q-pb-xs">
        ...
        <ul>
          <li>Buyer profile ID (yours): {{ entry.buyer_id }}.</li>
          <li>Seller profile ID: {{ entry.seller_id }}.</li>
...
onMounted(async () => {
  ...
  loaded.value = false;
  const res = await MARKETPLACE_API_IMP.getUserBuys();
  if (res.data) {
